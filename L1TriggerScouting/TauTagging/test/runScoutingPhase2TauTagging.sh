...
if [[ "$4" =~ ^(unpacking|clustering|sorting|reshaping|tagging)$ ]]; then
    TARGET=$1
else
...
if [ "${TARGET}" == "cpu" ]; then
  ...
  cmsRun "${SCRIPT}" runNumber=$3 buBaseDir=${DATA} fuBaseDir=${DATA} buNumStreams=2 maxEvents=$2 pipelineStep=$4 backend=serial_sync
elif [ "${TARGET}" == "cpu-dump" ]; then
  echo "Running CPU-only test with dumps"
  cmsRun "${SCRIPT}" runNumber=$3 buBaseDir=${DATA} fuBaseDir=${DATA} buNumStreams=2 maxEvents=$2 pipelineStep=$4 dump=candidates,cluster_indexes,soft_tau_inputs,soft_tau_outputs backend=serial_sync
elif [ "${TARGET}" == "cuda" ]; then
  echo "Running CUDA test"
  cmsRun "${SCRIPT}" runNumber=$3 buBaseDir=${DATA} fuBaseDir=${DATA} buNumStreams=2 maxEvents=$2 pipelineStep=$4 backend=cuda_async
elif [ "${TARGET}" == "cuda-dump" ]; then
  echo "Running CUDA test with dumps"
  cmsRun "${SCRIPT}" runNumber=$3 buBaseDir=${DATA} fuBaseDir=${DATA} buNumStreams=2 maxEvents=$2 pipelineStep=$4 dump=candidates,cluster_indexes,soft_tau_inputs,soft_tau_outputs backend=cuda_async
